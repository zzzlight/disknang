...
//========modify==========
  ...
  //     for (auto &v : gt) {
  //       if (res.find(v) != res.end()) {
  //         cur_recall+=recall_at;  //modified here
  //         break;                  //modified here
  //       }
  //     }
  //     total_recall += cur_recall;
 ...
 ////
  ...
  int build_merged_vamana_index(std::string     base_file,
    ...
    if (full_index_ram < ram_budget * 1024 * 1024 * 1024) {
      ...
      // return 0;
      std::cout<<" data_file save_graph K L iter S R RANGE PL B M  single index"<<std::endl;
       float* data_load = NULL;
      unsigned points_num, dim;
      
      efanna2e::new_load_data(base_file.c_str(), data_load, points_num, dim);
      ...
      rename(mem_index_path.c_str(),disk_index_path.c_str());
      // std::remove(medoids_file.c_str());
      // std::remove(centroids_file.c_str());
      return 0;
    }
...
//     //这里num_parts次建图
    ...
    */
      ...
      std::cout<<"point_num is"<<points_num<<" dim is"<<dim<<std::endl;
      efanna2e::IndexRandom init_index(dim, points_num);
      efanna2e::IndexGraph index(dim, points_num, efanna2e::L2, (efanna2e::Index*)(&init_index));
...
////////////////////////////////////
  ...
  template DISKANN_DLLEXPORT void create_disk_layout<int8_t>(
      ...
      const std::string output_file,unsigned R,double indexing_ram_budget);
