...
void gen_random_slice(const std::string data_file, double p_val,
  ...
  std::uniform_real_distribution<float> distribution(0, 1);
  //用种子生成随机数，范围0-1  p_val为150w除以10e  差不多就是通过这个办法抽取10e中的一部分数据
  for (size_t i = 0; i < npts; i++) {
...
int generate_pq_pivots(const float *passed_train_data, size_t num_train,
 ...
 // std::cout<<num_pq_chunks<<std::endl;
...
////////////////读入还有问题，问题出在fvecs是默认第一个是维度，而这里默认是128个数据
...
int shard_data_into_clusters(const std::string data_file, float *pivots,
  ...
  for (size_t i = 0; i < num_centers; i++) {
    ...
    shard_data_writer[i].write((char *) &dummy_size, sizeof(uint32_t));  //dummy 实际是一个占位  后面结束时候把点总数写在这
...
int partition_with_ram_budget(const std::string data_file,
  ...
  while (!fit_in_ram) {
    ...
    for (auto &p : cluster_sizes) {
      ...
      std::cout<<"in partation:"<<"p is "<<p<<" "<<"train dim is"<<train_dim<<std::endl;
